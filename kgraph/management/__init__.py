...
class ManagementUtility:
    ...
    def execute(self):
        ...
        parser.add_argument('-eps', '--epsilon', type=float, help='the mu distance between clusters in dengraph algorithm')
        parser.add_argument('-mu', '--mu', type=float, help='the epsilon-neighborhood of a node in dengraph algorithm')
        parser.add_argument('-c', '--compare-to', nargs='+', help='Algorithm(s) which main algorithm(s) is(are) compared')
        ...
        if args.component:
            print('INPUT: execute {0} algorithm(s) | Network: {1} vertices and {2} edges (largest component)'.format(' '.join(args.algorithms), g.num_vertices(), g.num_edges()))
        else:
            print('INPUT: execute {0} algorithm(s) | Network: {1} vertices and {2} edges'.format(' '.join(args.algorithms), g.num_vertices(), g.num_edges()))
...
def compute_results(graph, communities):
    ...
    print('Detected {0} communities with mean length = {1} and mean Ia = {2} and mean Ic = {3}'.format(int(len(communities)), length, Ia, Ic))
